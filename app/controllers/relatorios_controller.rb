class RelatoriosController < ApplicationController
  ...
  def professor
    ...
    for atribuicao in @atribuicao
        @aluno= Matricula.find(:all, :select => 'alunos.id, alunos.aluno_nome',:joins =>:aluno  ,:conditions=>['matriculas.classe_id=? AND (matriculas.status ="MATRICULADO" OR matriculas.status ="TRANSFERENCIA")', atribuicao.classe_id ], :order => 'alunos.aluno_nome ASC')
        if cont_cl==0
          @alunos2=@aluno
          cont_cl=cont_cl+1
        else
