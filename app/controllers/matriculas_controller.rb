...
t=0
    ...
    def unidade_transferencia
        ...
        @alunos = Aluno.find(:all, :select => "alunos.id, CONCAT(alunos.aluno_nome, ' | ', date_format(alunos.aluno_nascimento, '%d/%m/%Y'), ' | ', cl.classe_classe) AS aluno_nome_dtn", :joins =>"inner join matriculas mat on alunos.id=mat.aluno_id LEFT JOIN classes cl on mat.classe_id=cl.id" , :conditions =>['((alunos.unidade_id=52 AND mat.unidade_id=?) OR alunos.unidade_id=?) AND mat.ano_letivo=?', session[:unidade_ant_id], session[:unidade_ant_id], Time.now.year], :order => 'alunos.aluno_nome')
