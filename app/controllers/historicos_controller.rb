class HistoricosController < ApplicationController
    ...
    def destroy_nota
        ...
        @anos_letivos = Nota.find(:all, :select => 'notas.id, notas.ano_letivo, notas.matricula_id, notas.disciplina_id, notas.nota5, notas.classe', :joins =>[:matricula], :conditions => ["notas.aluno_id =? AND notas.classe IS NULL AND notas.ano_letivo<? AND (matriculas.reprovado='0') AND (matriculas.status='MATRICULADO' OR matriculas.status='TRANSFERENCIA' OR matriculas.status='*REMANEJADO' OR notas.matricula_id is NULL)", session[:aluno_id], Time.now.year], :order => "notas.ano_letivo ASC")
...
session[:id_classe_teste]=(@matricula.classe.id).to_s+" | "+@matricula.classe.classe_classe+" | "+@matricula.classe.horario
            ...
            @anos_letivos = Nota.find(:all, :select => 'notas.id, notas.ano_letivo, notas.matricula_id, notas.disciplina_id, notas.nota5, notas.classe', :joins =>"LEFT JOIN `matriculas` ON `matriculas`.id=`notas`.matricula_id", :conditions => ["notas.aluno_id =? AND notas.ano_letivo<? AND (matriculas.reprovado='0' OR notas.matricula_id is NULL) AND (matriculas.status='MATRICULADO' OR matriculas.status='TRANSFERENCIA' OR matriculas.status='*REMANEJADO' OR notas.matricula_id is NULL)", session[:aluno_id], Time.now.year], :order => "notas.ano_letivo ASC")
